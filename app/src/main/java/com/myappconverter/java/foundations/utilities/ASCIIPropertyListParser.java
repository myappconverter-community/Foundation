...
public class ASCIIPropertyListParser {
	...
	public static final char WHITESPACE_NEW_LINE = '\n';
	...
	private void skipWhitespacesAndComments() {
		...
		do {
			...
			// Skip whitespaces
			while (accept(WHITESPACE_CARRIAGE_RETURN, WHITESPACE_NEW_LINE, WHITESPACE_SPACE, WHITESPACE_TAB)) {
				skip();
			}
			...
			if (acceptSequence(COMMENT_BEGIN_TOKEN, SINGLELINE_COMMENT_SECOND_TOKEN)) {
				skip(2);
				readInputUntil(WHITESPACE_CARRIAGE_RETURN, WHITESPACE_NEW_LINE);
				commentSkipped = true;
			}
		...
		} while (commentSkipped); // if a comment was skipped more whitespace or another comment can follow, so skip
									// again
	}
	...
	/**
	 * Parses an array from the current parsing position. The prerequisite for calling this method is, that an array begin token has been
	 * read.
	 * 
	...
	/**
	 * Parses a dictionary from the current parsing position. The prerequisite for calling this method is, that a dictionary begin token has
	 * been read.
	 * 
	 * @return The dictionary found at the parsing position.
	 */
	private NSDictionary<String, NSObject> parseDictionary() throws ParseException {
		// Skip begin token
		skip();
		skipWhitespacesAndComments();
		NSDictionary<String, NSObject> dict = new NSDictionary<String, NSObject>();
		while (!accept(DICTIONARY_END_TOKEN)) {
			...
			dict.put(keyString, NSObject.wrap(object));
			skipWhitespacesAndComments();
	...
	/**
	 * Parses a data object from th current parsing position. This can either be a NSData object or a GnuStep NSNumber or NSDate. The
	 * prerequisite for calling this method is, that a data begin token has been read.
	 * 
	...
	/**
	 * Parses a plain string from the current parsing position. The string is made up of all characters to the next whitespace, delimiter
	 * token or assignment token.
	 * 
	 * @return The string found at the current parsing position.
	 */
	private String parseString() {
		String parsedString = readInputUntil(WHITESPACE_SPACE, WHITESPACE_TAB, WHITESPACE_NEW_LINE, WHITESPACE_CARRIAGE_RETURN,
				ARRAY_ITEM_DELIMITER_TOKEN, DICTIONARY_ITEM_DELIMITER_TOKEN, DICTIONARY_ASSIGN_TOKEN, ARRAY_END_TOKEN);
		return parsedString;
	}

	/**
	 * Parses a quoted string from the current parsing position. The prerequisite for calling this method is, that a quoted string begin
	 * token has been read.
	 * 
	...
	/**
	 * Parses a string according to the format specified for ASCII property lists. Such strings can contain escape sequences which are
	 * unescaped in this method.
	 * 
