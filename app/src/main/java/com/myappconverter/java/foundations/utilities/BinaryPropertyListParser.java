...
public class BinaryPropertyListParser {
	...
	public static NSObject parse(File f) throws IOException, PropertyListFormatException {
		if (f.length() > Runtime.getRuntime().freeMemory()) {
			throw new OutOfMemoryError("To little heap space available! Wanted to read " + f.length() + " bytes, but only "
					+ Runtime.getRuntime().freeMemory() + " are available.");
		}
		return parse(new FileInputStream(f));
	}

	/**
	 * Parses an object inside the currently parsed binary property list. For the format specification check <a
	 * href="http://www.opensource.apple.com/source/CF/CF-744/CFBinaryPList.c"> Apple's binary property list parser implementation</a>.
	 * 
	...
	private NSObject parseObject(int obj) throws IOException, PropertyListFormatException {
		...
		case 0x1: {
			...
			if (length < Runtime.getRuntime().freeMemory()) {
				...
				throw new OutOfMemoryError("To little heap space available! Wanted to read " + length + " bytes, but only "
						+ Runtime.getRuntime().freeMemory() + " are available.");
			}
		}
		case 0x2: {
		...
		case 0x3: {
			// Date
			if (objInfo != 0x3) {
				throw new PropertyListFormatException("The given binary property list contains a date object of an unknown type ("
						+ objInfo + ")");
			}
		...
		case 0x8: {
			...
			if (length < Runtime.getRuntime().freeMemory()) {// TODO
				// return new NSUUID(String.valueOf(obj), copyOfRange(bytes, offset + 1, offset + 1 + length));
				NSNumber nbr = new NSNumber(copyOfRange(bytes, offset + 1, offset + 1 + length), NSNumber.INTEGER);
				return new NSUUID(String.valueOf(nbr), copyOfRange(bytes, offset + 1, offset + 1 + length));
			} else {
				throw new OutOfMemoryError("To little heap space available! Wanted to read " + length + " bytes, but only "
						+ Runtime.getRuntime().freeMemory() + " are available.");
			}
		...
		case 0xA: {
			...
			for (int i = 0; i < length; i++) {
				int objRef = (int) parseUnsignedInt(copyOfRange(bytes, offset + arrayoffset + i * objectRefSize, offset + arrayoffset
						+ (i + 1) * objectRefSize));
				array.getWrappedList().add(i, parseObject(objRef));
			}
			return array;
		...
		case 0xB: {
			...
			for (int i = 0; i < length; i++) {
				int objRef = (int) parseUnsignedInt(copyOfRange(bytes, offset + contentOffset + i * objectRefSize, offset + contentOffset
						+ (i + 1) * objectRefSize));
		...
		case 0xD: {
			...
			for (int i = 0; i < length; i++) {
				int keyRef = (int) parseUnsignedInt(copyOfRange(bytes, offset + contentOffset + i * objectRefSize, offset + contentOffset
						+ (i + 1) * objectRefSize));
				int valRef = (int) parseUnsignedInt(copyOfRange(bytes, offset + contentOffset + (length * objectRefSize) + i
						* objectRefSize, offset + contentOffset + (length * objectRefSize) + (i + 1) * objectRefSize));
	...
	private int[] readLengthAndOffset(int objInfo, int offset) {
		...
		if (objInfo == 0xF) {
			...
			if (intType != 0x1) {
				System.err.println("BinaryPropertyListParser: Length integer has an unexpected type" + intType
