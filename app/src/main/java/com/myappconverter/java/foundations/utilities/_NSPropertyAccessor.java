...
import com.myappconverter.java.foundations.kvc.NSKeyValueCoding._KeyBinding;
import com.myappconverter.java.foundations.kvc.NSKeyValueCoding._KeyBindingCreation._KeyBindingFactory;
import com.myappconverter.java.foundations.kvc.NSKeyValueCoding._KeyBindingCreation._KeyBindingFactory._BindingStorage;

public class _NSPropertyAccessor {
	private static final _KeyBinding _NotAvailableIndicator = new _KeyBinding(null, null);
	private static final ConcurrentHashMap<_KeyBinding, _BindingStorage> _bindingStorageMapTable = new ConcurrentHashMap<_KeyBinding, _BindingStorage>(
			256);
	...
	public static boolean _canAccessFieldsDirectlyForClass(Class<?> objectClass) {
		// return
		// _NSReflectionUtilities._staticBooleanMethodValue("canAccessFieldsDirectly",
		// null, null, objectClass, NSKeyValueCoding.class, true);
		return true;
	}
	...
	private _KeyBinding _createKeyBindingForKey(String key, int[] lookupOrder, boolean trueForSetAndFalseForGet) {
		...
		// we use a KeyBinding as key for the _BindingStorage object map table
		// since it gives us exactly what we need: a class and a key - but we
		// have to create a new lookup key binding to avoid synchronizing the
		// read lookup (and we need a new instance for the write access)
		...
		_KeyBindingFactory.Callback keyBindingCreationCallbackObject = (targetObject instanceof _KeyBindingFactory.Callback) ? (_KeyBindingFactory.Callback) targetObject
				: null;
		_KeyBinding keyBindings[] = (trueForSetAndFalseForGet) ? bindingStorage._keySetBindings
				: bindingStorage._keyGetBindings;
		for (int i = 0; i < lookupOrder.length; i++) {
			int lookup = lookupOrder[i];
			_KeyBinding keyBinding = ((lookup >= _KeyBindingFactory.MethodLookup) && (lookup <= _KeyBindingFactory.UnderbarFieldLookup)) ? keyBindings[lookup]
					: null;
			if (keyBinding == null) {
				...
				case _KeyBindingFactory.MethodLookup:
					...
					if (trueForSetAndFalseForGet) {
						// look up 'setKey'
						keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject._methodKeySetBinding(
								key, methodName) : _methodKeySetBinding(key, methodName, valueType);
					} else {
						// look up 'getKey'
						keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject._methodKeyGetBinding(
								key, methodName) : _methodKeyGetBinding(key, methodName);

						if (keyBinding == null) {
							// look up 'key'
							keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject
									._methodKeyGetBinding(key, key) : _methodKeyGetBinding(key, key);
						}
						...
						if (keyBinding == null) {
							...
							keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject
									._methodKeyGetBinding(key, methodName) : _methodKeyGetBinding(key, methodName);
						}
					}
					break;
				case _KeyBindingFactory.UnderbarMethodLookup:
					...
					if (trueForSetAndFalseForGet) {
						// look up '_setKey'
						keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject._methodKeySetBinding(
								key, underbarMethodName) : _methodKeySetBinding(key, underbarMethodName, valueType);
					} else {
						// look up '_getKey'
						keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject._methodKeyGetBinding(
								key, underbarMethodName) : _methodKeyGetBinding(key, underbarMethodName);

						if (keyBinding == null) {
							// look up '_key'
							underbarMethodName = prefixedKey("_", key);
							keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject
									._methodKeyGetBinding(key, underbarMethodName) : _methodKeyGetBinding(key, underbarMethodName);
						}
				...
				case _KeyBindingFactory.FieldLookup:
					...
					if (canAccessFieldsDirectly) {
						// look up 'key'
						keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject._fieldKeyBinding(key,
								key) : _fieldKeyBinding(key, key);

						if (keyBinding == null) {
							// look up 'isKey'
							String fieldName = prefixedKey("is", key);
							keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject._fieldKeyBinding(
									key, fieldName) : _fieldKeyBinding(key, fieldName);
						}
					}
					break;
				case _KeyBindingFactory.UnderbarFieldLookup:
					...
					if (canAccessFieldsDirectly) {
						...
						keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject._fieldKeyBinding(key,
								underbarFieldName) : _fieldKeyBinding(key, underbarFieldName);

						if (keyBinding == null) {
							// look up '_isKey'
							underbarFieldName = prefixedKey("_is", key);
							keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject._fieldKeyBinding(
									key, underbarFieldName) : _fieldKeyBinding(key, underbarFieldName);
						}
					}
					break;
				case _KeyBindingFactory.OtherStorageLookup:
					keyBinding = (keyBindingCreationCallbackObject != null) ? keyBindingCreationCallbackObject._otherStorageBinding(key)
							: null;
					break;
				}
				...
				if ((lookup == _KeyBindingFactory.FieldLookup) || (lookup == _KeyBindingFactory.UnderbarFieldLookup)) {
					// set and get bindings are the same for fields (but not for
					// methods since the name of set and get methods are
					// actually different)
	...
	public _KeyBinding _fieldKeyBinding(String key, String fieldName) {
		Class<?> objectClass = targetObject.getClass();
		...
		try {
			...
			if ((publicFieldOnly && !AccessFlag.isPublic(field.getModifiers())) || AccessFlag.isPrivate((field.getModifiers()))) {
				return null;
			}
			...
			CtMethod getter = CtMethod.make(
					"public Object valueInObject(Object object) {" + "return "
							+ box(valueType, unbox(targetCtClass, "object") + "." + fieldName) + "; }", wrapper);
	...
	public _KeyBinding _methodKeyGetBinding(String key, String methodName) {
		Class<?> objectClass = targetObject.getClass();
		NSKeyValueCoding.ValueAccessor valueAccessor = NSKeyValueCoding.ValueAccessor._valueAccessorForClass(objectClass);
		...
		try {
			...
			for (CtMethod target : targetCtClass.getMethods()) {
				if (!target.getName().equals(methodName) || target.getParameterTypes().length != 0) {
					continue;
				}
				if ((publicMethodOnly && !AccessFlag.isPublic(target.getModifiers())) || (AccessFlag.isPrivate(target.getModifiers()))) {
					continue;
				}
				method = target;
				break;
			...
			CtMethod getter = CtMethod.make(
					"public Object valueInObject(Object object) {" + "return "
							+ box(valueType, unbox(method.getDeclaringClass(), "object") + "." + methodName + "()") + "; }", wrapper);
	...
	public _KeyBinding _methodKeySetBinding(String key, String methodName, Class<?> targetValueType) {
		Class<?> objectClass = targetObject.getClass();
		NSKeyValueCoding.ValueAccessor valueAccessor = NSKeyValueCoding.ValueAccessor._valueAccessorForClass(objectClass);
		boolean publicMethodOnly = (valueAccessor == null);
	...
	private String convert(CtClass type, String arg) {
		String converted = "(" + _NSPropertyUtilities.class.getName() + ".convertObjectIntoCompatibleValue(" + arg + ","
				+ boxedTypeName(type) + ".class" + "))";
		boolean isPrimitive = type.isPrimitive();
		if (isPrimitive && (type == CtClass.intType))
