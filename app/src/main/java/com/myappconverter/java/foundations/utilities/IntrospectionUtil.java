...
public class IntrospectionUtil {
	...
	public static Method findMethod(MACFuntionPointer functionPointer) throws NoSuchMethodException {

		return findMethod(functionPointer.clazz, functionPointer.methodName, functionPointer.args, false, true);
	}
	...
	public static Method findMethod(Class clazz, String methodName, Class[] args, boolean checkInheritance, boolean strictArgs)
		...
		for (int i = 0; i < methods.length && method == null; i++) {
			if (methods[i].getName().equals(methodName)
					&& checkParams(methods[i].getParameterTypes(), (args == null ? new Class[] {} : args), strictArgs)) {
				method = methods[i];
			}
	...
	public static Field findField(Class clazz, String targetName, Class targetType, boolean checkInheritance, boolean strictType)
		...
		try {
			...
			if (checkInheritance) {
				...
				throw new NoSuchFieldException("No field with name " + targetName + " in class " + clazz.getName() + " of type "
						+ targetType);
